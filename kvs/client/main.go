...
func (client *Client) TxGetRPC(txid string, key string) (string, bool) {
	...
	if err := client.rpcClient.Call("KVService.TxGet", &request, &response); err != nil {
		return "", false
	}
...
func (client *Client) TxPutRPC(txid string, key, value string) bool {
	...
	if err := client.rpcClient.Call("KVService.TxPut", &request, &response); err != nil {
		return false
	}
	return response.Ok
}
...
func (client *Client) TxCommitRPC(txid string, lead bool) bool {
	...
	if err := client.rpcClient.Call("KVService.TxCommit", &request, &response); err != nil {
		return false
	}
	return response.Ok
}

func (client *Client) TxAbortRPC(txid string) bool {
	...
	if err := client.rpcClient.Call("KVService.TxAbort", &request, &response); err != nil {
		return false
	}
	return true
}
...
func runClient_Txn(id int, addrs []string, done *atomic.Bool, workload *kvs.Workload, resultsCh chan<- uint64) {
	txnSize := 3
	batchSize := 4096 //only for initializing the queue size
	ttlFlush := time.Millisecond
